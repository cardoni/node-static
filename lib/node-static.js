...
var Server = function (root, options) {
    ...
    this.options.indexFilePrefs = this.options.indexFilePrefs || [ 'index.html', 'index.htm' ];
...
Server.prototype.serveDir = function (pathname, req, res, finish) {
    if ( this.options.indexFilePrefs && !Array.isArray( this.options.indexFilePrefs ) ) {
        this.options.indexFilePrefs = new Array( this.options.indexFilePrefs );
    }

    var htmlIndexFiles = this.options.indexFilePrefs.map( indexFile => {
      return path.join( pathname, indexFile );
    } );
    var that = this;

    tryIndexFileType();

    function tryIndexFileType() {
      htmlIndexFiles.forEach( htmlIndexFile => {
        fs.stat( htmlIndexFile, ( e, stat ) => {
          if (!e) {
            var status = 200;
            var headers = {};
            ...
            if (originalPathname.length && originalPathname.charAt(originalPathname.length - 1) !== '/') {
              ...
              that.respond(null, status, headers, [htmlIndexFile], stat, req, res, finish);
            }
          } else {

            // if we have more to try, try the next one...
            if ( htmlIndexFiles && htmlIndexFiles.length > 1 ) {
              // remove the index file ext that didn't exist...
              // delete htmlIndexFiles[ htmlIndexFile ];
              var lastTriedFile = htmlIndexFiles.indexOf( htmlIndexFile );
              if ( lastTriedFile > -1 ) {
                  htmlIndexFiles.splice( lastTriedFile, 1 );
              }
              return tryIndexFileType();
            }
